...
import '../utils/platform_utils.dart'; // used for !isMobile (Messenger on web/desktop only)
...
import 'paper_timesheet_screen.dart';
...
import 'message_log_screen.dart';
...
class _MainMenuScreenState extends State<MainMenuScreen> {
  ...
  Future<void> _loadMenuData() async {
    ...
    try {
      // Load user setup and permissions (pass setup to avoid duplicate getCurrentUserSetup call)
    ...
    if (_isLargeScreen) {
      ...
      return Scaffold(
        appBar: AppBar(
          ...
          actions: [
            const ScreenInfoIcon(screenName: 'main_menu_screen.dart'),
            // User info icon
            IconButton(
  ...
  Widget _buildMenuList() {
    return ListView(
      ...
      children: [
        ...
        // Messenger submenu - Web/Windows only (excluded from Android/iOS builds)
        if (_menuPermissions[UserService.menuMessenger] == true && !isMobile)
          _buildMenuSection(
            key: 'messenger',
            title: 'Messenger',
            icon: Icons.mail,
            subItems: [
              _SubMenuItem(
                title: 'New Message',
                onTap: () => _navigateToScreen(const NewMessageScreen()),
              ),
              _SubMenuItem(
                title: 'Message Log',
                onTap: () => _navigateToScreen(MessageLogScreen()),
              ),
              _SubMenuItem(
                title: 'Message Template',
                onTap: () => _navigateToScreen(MessageTemplateScreen()),
              ),
            ],
          ),
        ...
        if (_menuPermissions[UserService.menuManagers] == true)
          _buildMenuSection(
            ...
            subItems: [
              ...
              _SubMenuItem(
                title: 'Paper Timesheet',
                onTap: () => _navigateToScreen(const PaperTimesheetScreen()),
              ),
              _SubMenuItem(
