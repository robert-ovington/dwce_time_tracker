/// User Setup (PPE Management) – PPE Manager creates a PPE request on behalf of any user.
/// Bypasses manager approval: request is created with status approved.
...
class PpeUserSetupScreen extends StatefulWidget {
  const PpeUserSetupScreen({super.key});

  @override
  State<PpeUserSetupScreen> createState() => _PpeUserSetupScreenState();
}
...
class _PpeUserSetupScreenState extends State<PpeUserSetupScreen> {
  ...
  String? _selectedSizeCatalogId;
  /// Saved when an item is added; used to prefill size when a new PPE is selected (if valid for that PPE).
  String? _savedSizeCatalogId;
  final _qtyController = TextEditingController(text: '1');
  final _reasonController = TextEditingController();
  bool _saving = false;
  /// Line items: ppe_id, ppe_name, size_id, size_code (display), quantity, reason.
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      _users = usersList.map((e) => Map<String, dynamic>.from(e as Map)).toList();
      _users.sort((a, b) => (a['display_name']?.toString() ?? '').compareTo(b['display_name']?.toString() ?? ''));
      ...
      ErrorLogService.logError(location: 'PPE User Setup - Load', type: 'Database', description: '$e', stackTrace: st);
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          ...
          _selectedSizeCatalogId = _savedSizeCatalogId != null && options.any((o) => o.id == _savedSizeCatalogId) ? _savedSizeCatalogId : null;
        });
      }
    } catch (e, st) {
      ErrorLogService.logError(location: 'PPE User Setup - Load sizes', type: 'Database', description: '$e', stackTrace: st);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('$e'), backgroundColor: Colors.red));
        setState(() {
          _sizesForSelected = [];
          _loadingSizes = false;
          _selectedSizeCatalogId = null;
        });
      }
    }
  }
  ...
  void _addToList() {
    ...
    for (final o in _sizesForSelected) {
      if (o.id == _selectedSizeCatalogId) { opt = o; break; }
    }
    ...
    setState(() {
      ...
      _savedSizeCatalogId = sizeOpt.id;
      _selectedPpeId = null;
    });
  }
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('${_lineItems.length} request(s) created (approved)'), backgroundColor: Colors.green));
        _lineItems.clear();
        _qtyController.text = '1';
        _reasonController.clear();
      }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: const [ScreenInfoIcon(screenName: 'ppe_user_setup_screen.dart')],
      ),
        ...
        child: Column(
          ...
          children: [
            _buildSectionCard(
              ...
              children: [
                const Text(
                  'Create a PPE request on behalf of a user (bypasses manager approval).',
            ...
            _buildSectionCard(
              ...
              children: [
                ...
                _loadingSizes
                    ...
                    : DropdownButtonFormField<String>(
                        value: _sizesForSelected.any((o) => o.id == _selectedSizeCatalogId) ? _selectedSizeCatalogId : null,
                        ...
                        onChanged: _sizesForSelected.isEmpty ? null : (v) => setState(() => _selectedSizeCatalogId = v),
                      ),
                const SizedBox(height: 12),
                TextField(
